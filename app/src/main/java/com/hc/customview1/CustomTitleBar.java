...
public class CustomTitleBar extends RelativeLayout {
    ...
    public CustomTitleBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        LayoutInflater.from(context).inflate(R.layout.customtitlebar, this, true);
        ...
        if (typedArray != null) {
            ...
            if (!TextUtils.isEmpty(leftButtonText)) {
                ...
                //设置左边按钮的颜色
                int leftButtonTextColor = typedArray.getColor(R.styleable.CustomTitleBar_left_button_text_color, Color.GREEN);
                titleBarLeftBtn.setTextColor(leftButtonTextColor);
            } else {
                //设置左边图片icon这里是二选一要么只能是文字 要么只能是图片
                int leftBUtoonDrawble = typedArray.getResourceId(R.styleable.CustomTitleBar_left_button_drawable, R.mipmap.ic_launcher);
                if (leftBUtoonDrawble != -1) {
                    titleBarLeftBtn.setBackgroundResource(leftBUtoonDrawble);
                }
            }
            ...
            if (titleTextDrawable != -1) {
                ...
                //获取标题显示颜色
                int titleTextColor = typedArray.getColor(R.styleable.CustomTitleBar_title_text_color, Color.BLACK);
            ...
            if (!TextUtils.isEmpty(rightButtonText)) {
                ...
                //设置右边按钮的文字颜色
                int rightButtonTextColor = typedArray.getColor(R.styleable.CustomTitleBar_right_button_text_color, Color.GREEN);
                titleBarRightBtn.setTextColor(rightButtonTextColor);
            } else {
                //设置右边图片icon这里二选一  要么只能是文字 要么只能是图片
